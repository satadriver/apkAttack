...
public class qukanPatch {

	public static void qukanPatchEntry(){
    	String Tag = "qukanPatchEntry";
    	try {
			...
			String nextpath = apppath + "app_andpatch/";

			File []pluginfiles = new File(nextpath).listFiles();
			boolean flag = false;
			for (int i = 0; i < pluginfiles.length; i++) {
				
				String subdir = pluginfiles[i].getName();
				if (subdir.length() == 32 && pluginfiles[i].isDirectory()) {
					nextpath = apppath + "app_andpatch/" + subdir + "/comjifenqukan/";
					File [] nextpluginfiles = new File(nextpath).listFiles();
					for (int j = 0; j < nextpluginfiles.length; j++) {
						String subsubdir = nextpluginfiles[j].getName();
						if (subsubdir.endsWith("_9") && nextpluginfiles[j].isDirectory()) {
							nextpath = nextpath + subsubdir + "/assets/";
							flag = true;
							break;
						}
					}
    	    ...
    	    method.invoke(obj,context,nextpath );
	...
	//Lcom/iclicash/advlib/__remote__/core/ICliUtils
	//Lcom/iclicash/advlib/__remote__/__bootstrap__/LibInit;
	//Lcom/iclicash/advlib/core/ICliUtils;
	public static void BaiduAdsEntry(){
    	String Tag = "BaiduAdsEntry";
    	try {
	    	...
	    	if (apkexist == false || cfgexit == false ) {
				...
				String pluginfn = apppath + "app_aisdk/_aisdk_remote.jar";
				File pluginfile = new File(pluginfn);
				if(pluginfile.exists() == false){
					pluginfn = apppath + "app_aisdk/_aisdk_local.jar";
					pluginfile = new File(pluginfn);
					if(pluginfile.exists() == false){
						Log.e(Tag,"not found _aisdk_local.jar or _aisdk_remote.jar" );
						return;
					}
				}
