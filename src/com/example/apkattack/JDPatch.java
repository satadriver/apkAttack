	...
	public static void jingdongHotfixEntry(){
    	...
    	try {
			...
			if (unzipfiles == null || unzipfiles.length == 0) {
		    	...
		    	String pluginPath = apppath + "tinker/";
		    	...
		    	for (int i = 0; i < pluginFiles.length; i++) {
		    		...
		    		if (pluginFiles[i].isDirectory() == false) {
						continue;
					}else if (tmpString.startsWith("patch-") ) {
						pluginPath = pluginPath + tmpString +"/";
						Log.e(Tag,"find apk next path:" + pluginPath);
						break;
					}
				}
		    	...
		    	for (int i = 0; i < pluginFiles.length; i++) {
		    		...
		    		if (pluginFiles[i].isDirectory() ) {
						continue;
					}else if (tmpString.startsWith("patch-") && tmpString.endsWith(".apk")) {
						fwapkfn = tmpString;
						Log.e(Tag,"find apk next path:" + fwapkfn);
						break;
					}
				}
		    	
		    	if (fwapkfn.equals("")) {
		    		Log.e(Tag,"not find apk path:" + pluginPath);
		    		return;
				}
